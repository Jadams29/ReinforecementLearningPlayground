...
def getVKBindings():
	...
	for i in range(48, 91):
		bindings[f"VK_{chr(i)}"] = hex(ord(chr(i)))
		
	for i in range(0, 10):
		bindings[f"VK_NUMPAD{i}"] = f"0x6{i}"
		
	bindings[f"VK_UP"] = f"0x26"
	bindings[f"VK_LEFT"] = f"0x25"
	bindings[f"VK_DOWN"] = f"0x28"
	bindings[f"VK_RIGHT"] = f"0x27"
